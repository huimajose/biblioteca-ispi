...
export default function SuggestedBooks({ userId }: SuggestedBooksProps) {
  ...
  useEffect(() => {
    ...
    const fetchBooks = async () => {
      try {
        // 1️⃣ Verifica se é estudante
        const roleRes = await fetch(`/api/user-role/${userId}`);
        if (!roleRes.ok) throw new Error("Erro ao verificar role");
        const roleData = await roleRes.json();

        if (!roleData.isStudent) {
          setBlocked(true);
          setLoading(false);
          return;
        }

        // 2️⃣ Buscar sugestões de livros
  ...
  if (loading)
    return (
      <div className="bg-white rounded-lg shadow p-6 mt-6 flex flex-col items-center justify-center">
        ...
        <h2 className="text-xl font-semibold text-green-600">Carregando sugestões...</h2>
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6 mt-6 flex flex-col items-center justify-center">
        <h2 className="text-xl font-semibold text-red-600">
          Somente estudantes podem ver sugestões de livros, clique aqui para verificar <a href="/user/student-verification">aqui</a>.
        </h2>
      </div>
    );
    ...
    <div className="bg-white rounded-lg shadow p-6 mt-6">
      ...
      <ul className="space-y-2">
        {books.map((book) => (
          <li
            key={book.id}
            className="p-2 border rounded hover:bg-green-50 transition"
          >
